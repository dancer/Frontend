          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <div className="font-medium flex items-center gap-2">
                {tournament.name}
                {tournament.id === "ucl" && <Trophy className="h-4 w-4 text-yellow-500" />}
              </div>
              <div className="text-xs text-zinc-400">
                {tournament.region} • {tournament.matchCount} matches
              </div>
            </div>
            <div className="ml-auto bg-zinc-800/70 px-3 py-1 rounded-full text-sm">{tournament.matchCount}</div>
          </div>

          <div className="flex flex-wrap gap-2">
            {tournament.teams.map((team, index) => (
              <span key={index} className="text-xs bg-zinc-800/50 px-2 py-1 rounded-full text-zinc-400">
                {team}
              </span>
            ))}
          </div>
